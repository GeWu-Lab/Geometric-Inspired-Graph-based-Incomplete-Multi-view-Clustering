...
Our method can approximate the full-view solution, denoted as $\textbf{s}^*_{(3)}$, using just available views. We obtain the optimal solution $\textbf{s}^*_{(2)}$ on available views through projection, which has the highest cosine similarity to the full-view solution.
